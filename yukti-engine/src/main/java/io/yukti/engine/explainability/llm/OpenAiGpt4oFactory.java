...
public final class OpenAiGpt4oFactory implements LlmProviderFactory {
    @Override public LlmProviderId id() { return LlmProviderId.OPENAI_GPT4O; }

    @Override public LlmProvider createOrNull() {
        String key = System.getenv(OpenAiLlmProvider.ENV_OPENAI_API_KEY);
        if (key == null || key.isBlank()) return null;
        return new OpenAiLlmProvider(key, "gpt-4o");
    }
}
