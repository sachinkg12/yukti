...
class CapAwareGreedyOptimizerTest {
    ...
    void optimize_deterministic() {
        Map<Category, Money> amounts = Map.of(Category.GROCERIES, Money.usd(500), Category.DINING, Money.usd(250), Category.OTHER, Money.usd(400));
        SpendProfile profile = new SpendProfile(Period.MONTHLY, amounts);
        OptimizationRequest request = new OptimizationRequest(profile, UserGoal.of(GoalType.CASHBACK), UserConstraints.defaults(), Map.of());
    ...
    void optimize_maxAnnualFeeEnforced() {
        ...
        assertTrue(r.getBreakdown().getFees().getAmount().compareTo(java.math.BigDecimal.valueOf(100)) <= 0,
            "CapAwareGreedyOptimizer must enforce fee budget: total fees must not exceed maxAnnualFee=$100, got " + r.getBreakdown().getFees());
    }
}
